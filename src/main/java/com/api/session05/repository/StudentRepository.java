...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query("select new com.api.session05.model.dto.response.student_response.StudentResponse(s.name, s.email) from Student s where (:studentName is null or lower(s.name) like concat('%', concat(lower(cast(:studentName as string )) , '%')))")
    Page<StudentResponse> searchStudentByName(@Param("studentName") String studentName, Pageable pageable);
}
